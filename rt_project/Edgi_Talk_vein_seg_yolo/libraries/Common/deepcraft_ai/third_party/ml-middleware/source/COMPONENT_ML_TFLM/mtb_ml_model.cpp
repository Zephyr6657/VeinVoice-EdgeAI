...
cy_rslt_t mtb_ml_model_init(const mtb_ml_model_bin_t *bin, const mtb_ml_model_buffer_t *buffer, mtb_ml_model_t **object)
{
    ...
    /*
     * This RT-Thread project links with -nostartfiles, so C++ global
     * constructors are not guaranteed to run. Construct the resolver on first
     * model init instead of relying on a global static AllOpsResolver object.
     */
    if (tflite::resolver == nullptr)
    {
        tflite::resolver = new tflite::AllOpsResolver();
        if (tflite::resolver == nullptr)
        {
