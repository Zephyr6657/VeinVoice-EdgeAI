...
static rt_device_t g_uvc_lcd_device;
...
static UVC_ITCM_SECTION rt_err_t uvc_display_flush(const struct uvc_update_rect *rect)
{
    ...
    if ((!g_uvc_lcd_hw_ready) || (g_uvc_lcd_device == RT_NULL)) {
        return -RT_ERROR;
    }
    ...
    if ((rect != RT_NULL) && (rect->width > 0U) && (rect->height > 0U)) {
        rect_info.x = rect->x;
        rect_info.y = rect->y;
        rect_info.width = rect->width;
        rect_info.height = rect->height;
        return rt_device_control(g_uvc_lcd_device, RTGRAPHIC_CTRL_RECT_UPDATE, &rect_info);
    }

    return rt_device_control(g_uvc_lcd_device, RTGRAPHIC_CTRL_RECT_UPDATE, RT_NULL);
}

static void uvc_display_draw_startup_pattern(void)
{
    uint32_t y;
    ...
    for (y = 0U; y < LCD_H; y++) {
        uint16_t color = 0x0841U;
        uint16_t *row = lcd_fb + y * LCD_W;
        uint32_t x;

        if (y < 80U) {
            color = 0x0210U;
        } else if ((y >= 220U) && (y < 580U)) {
            color = 0x18E3U;
        }
...
static UVC_ITCM_SECTION rt_err_t uvc_display_hw_init(void)
{
    ...
    g_uvc_lcd_device = rt_device_find("lcd");
    if (g_uvc_lcd_device == RT_NULL) {
        USB_LOG_ERR("LCD device not found\r\n");
        return -RT_ERROR;
    }
    ...
    if ((rt_device_control(g_uvc_lcd_device, RTGRAPHIC_CTRL_GET_INFO, &info) == RT_EOK) &&
        (info.framebuffer != RT_NULL) && (info.smem_len > 0U)) {
    ...
    if (!g_uvc_display_ctx.viewport_enabled) {
        uvc_display_draw_startup_pattern();
...
void uvc_display_set_viewport(uint16_t x, uint16_t y, uint16_t width, uint16_t height)
{
    ...
    if ((x >= LCD_W) || (y >= LCD_H) || (width == 0U) || (height == 0U)) {
        ctx->viewport_enabled = RT_FALSE;
        ctx->prepared = RT_FALSE;
        ctx->background_synced = RT_FALSE;
        return;
    }

    if (((uint32_t)x + width) > LCD_W) {
        width = (uint16_t)(LCD_W - x);
    }
    if (((uint32_t)y + height) > LCD_H) {
        height = (uint16_t)(LCD_H - y);
    }
...
int uvc_display_get_viewport(uint16_t *x, uint16_t *y, uint16_t *width, uint16_t *height)
{
    ...
    if (width != RT_NULL) {
        *width = ctx->viewport_w;
    }
    if (height != RT_NULL) {
        *height = ctx->viewport_h;
    }
...
void uvc_display_clear_viewport(uint16_t color_rgb565)
{
    ...
    for (uint16_t y = 0U; y < ctx->viewport_h; y++) {
        uint16_t *row = lcd_fb + ((uint32_t)ctx->viewport_y + y) * LCD_W + ctx->viewport_x;
        for (uint16_t x = 0U; x < ctx->viewport_w; x++) {
            row[x] = color_rgb565;
        }
    }

    rect.x = ctx->viewport_x;
    rect.y = ctx->viewport_y;
    rect.width = ctx->viewport_w;
    rect.height = ctx->viewport_h;
    (void)uvc_display_flush(&rect);
}
...
static UVC_ITCM_SECTION rt_err_t uvc_display_prepare_geometry(uint16_t src_w, uint16_t src_h)
{
    ...
    if (ctx->viewport_enabled) {
        area_x = ctx->viewport_x;
        area_y = ctx->viewport_y;
        area_w = ctx->viewport_w;
        area_h = ctx->viewport_h;
    } else {
        area_x = 0U;
        area_y = 0U;
        area_w = LCD_W;
    ...
    scale_h = ((uint32_t)src_h * area_w) / src_w;
    if ((scale_h > 0U) && (scale_h <= area_h)) {
        ctx->dst_w = area_w;
        ctx->dst_h = (uint16_t)scale_h;
    } else {
        scale_w = ((uint32_t)src_w * area_h) / src_h;
        if (scale_w == 0U) {
            scale_w = 1U;
        }
        if (scale_w > area_w) {
    ...
    ctx->x_offset = (uint16_t)(area_x + ((area_w - ctx->dst_w) / 2U));
    ctx->y_offset = (uint16_t)(area_y + ((area_h - ctx->dst_h) / 2U));

    ctx->update_rect.x = area_x;
    ctx->update_rect.y = area_y;
    ctx->update_rect.width = area_w;
    ctx->update_rect.height = area_h;
...
static UVC_ITCM_SECTION void uvc_display_sync_background(void)
{
    ...
    if (!ctx->background_synced) {
        ...
        if (ctx->viewport_enabled) {
            for (uint16_t y = 0U; y < ctx->viewport_h; y++) {
                uint16_t *row = lcd_fb + ((uint32_t)ctx->viewport_y + y) * LCD_W + ctx->viewport_x;
                memset(row, 0, (size_t)ctx->viewport_w * sizeof(uint16_t));
            }
            (void)uvc_display_flush(&ctx->update_rect);
        } else {
            memset(lcd_fb, 0, lcd_fb_size);
            (void)uvc_display_flush(RT_NULL);
        }

        ctx->background_synced = RT_TRUE;
    }
}

static UVC_ITCM_SECTION rt_bool_t uvc_display_rgb565(const uint16_t *src, uint32_t src_size,
                                                     uint16_t src_w, uint16_t src_h)
{
    ...
    for (uint16_t dy = 0; dy < ctx->dst_h; dy++) {
        ...
        drow = lcd_fb + ((uint32_t)ctx->y_offset + dy) * LCD_W + ctx->x_offset;
 ...
 */
static UVC_ITCM_SECTION rt_bool_t uvc_display_yuyv(const uint8_t *src, uint32_t src_size,
                                                   uint16_t src_w, uint16_t src_h)
{
    struct uvc_display_context *ctx = &g_uvc_display_ctx;
    ...
    if ((src_w == UVC_YUYV_STAGE_W) && (src_h == UVC_YUYV_STAGE_H)) {
        ...
        return uvc_display_rgb565(g_uvc_yuyv_stage_rgb565,
...
int uvc_display_init(void)
{
    ...
    USB_LOG_INFO("LCD: %ux%u %dbpp fb=%p size=%u\r\n",
                 LCD_W, LCD_H, 16,
                 lcd_fb, lcd_fb_size);
    USB_LOG_INFO("LCD viewport: %s x=%u y=%u w=%u h=%u\r\n",
                 g_uvc_display_ctx.viewport_enabled ? "on" : "off",
                 g_uvc_display_ctx.viewport_x,
                 g_uvc_display_ctx.viewport_y,
                 g_uvc_display_ctx.viewport_w,
                 g_uvc_display_ctx.viewport_h);
    return 0;
}
...
static void uvc_display_run_overlay_callback(void)
{
    ...
    info.dst_x_offset = g_uvc_display_ctx.x_offset;
                                        ...
                                        uint16_t src_w, uint16_t src_h)
{
    ...
    if (frame->frame_format == USBH_VIDEO_FORMAT_MJPEG) {
        ...
        if (!uvc_display_rgb565(g_uvc_mjpeg_rgb565,
    ...
    /* YUYV rendering */
    if (!uvc_display_yuyv(frame->frame_buf, frame->frame_size, src_w, src_h)) {
        return;
    }
