...
whd_result_t whd_bus_sdio_init(whd_driver_t whd_driver)
{
    ...
    if (loop_count >= (uint32_t)F2_READY_TIMEOUT_MS)
    {
         ...
         * crystal frequency.
         */
        WPRINT_WHD_ERROR( ("F2 ready diagnostic: result=%" PRIu32 " iordy=0x%02x loops=%" PRIu32 "\n",
                           (uint32_t)result, byte_data, loop_count) );
...
static whd_result_t whd_bus_sdio_write_wifi_nvram_image(whd_driver_t whd_driver)
{
    ...
    WPRINT_WHD_INFO( ("NVRAM download diagnostic: raw=%" PRIu32 " aligned=%" PRIu32
                      " ram_size=0x%08" PRIx32 " atcm=0x%08" PRIx32
                      " base=0x%08" PRIx32 " end=0x%08" PRIx32 "\n",
                      raw_image_size, image_size,
                      (uint32_t)GET_C_VAR(whd_driver, CHIP_RAM_SIZE),
                      (uint32_t)GET_C_VAR(whd_driver, ATCM_RAM_BASE_ADDRESS),
                      img_base,
                      (uint32_t)(img_end + GET_C_VAR(whd_driver, ATCM_RAM_BASE_ADDRESS))) );
